# Script to evaluate success rate based on 5 ranking categories (>3s, >2s, >1s, >0s, failure), 
# see paper Fig. 6 and Table II for more details
import os
import json
import numpy as np
import pandas as pd
...
def make_parser():
    ...
    parser.add_argument('--input_dir', type=str, help='Directory of Grasp Information',
                        default='/data/net/userstore/leyong.loh/git/isaac_sim_grasping/Dataset/hithand_filtered')
    parser.add_argument('--output_dir', type=str, help='Directory of Gripper urdf/usd',
                        default="/data/net/userstore/leyong.loh/git/isaac_sim_grasping/results")
...
def eval():
    ...
    last_row = len(df)
    df.loc[last_row, columns_names[1:6]] = df.mean()[columns_names[1:6]]
    df.loc[last_row, columns_names[-1]] = df.sum()[columns_names[-1]]
